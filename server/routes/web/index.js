module.exports = app => {
  ...
  router.get('/news/init', async (req, res) => {
    ...
    const cats = await Category.where({parents: parent}).find().lean()
    const newsTitles =  ["新皮肤爆料｜掌控万物，武则天化身宇宙女王！", "新皮肤爆料丨守护皇后，黑桃队长鲁班七号待命中", "新星元套装爆料丨大小姐驾到！未来机甲高能预警~", "过年有王者！新春相约峡谷，与你开启福气年", "和“王者荣耀”英雄组团，逛京东，贺新年！", "1月21日全服不停机更新公告", "1月20日体验服停机更新公告", "游戏内限时活动入口无法显示异常说明", "组队消费活动微信异常已修复", "1月16日体验服停机更新公告", "金鼠送礼 峡谷新春福利来袭", "组队消费活动消费期开启说明", "【蔷薇的历练之年兽大作战】活动公告及FAQ", "内测皮肤获取方式升级【蔷薇的历练】开启", "凤求凰&凤凰于飞限时返场 皮肤优化即将来袭", "假期狂欢季开启，高校区域联赛战队携手虎牙人气主播开战啦！", "高校自制定格动画：说出来你可能不信，我们的“作业”打起来了！", "2020年KPL春季转会期俱乐部挂牌名单公布", "假期狂欢季线上赏金对抗赛即将开启，这次由你当“教练”！", "历时72小时打造东北版《王者荣耀》，雪雕王者经典形象"]
    
    const newsList = newsTitles.map(i => {
  ...
  router.get('/news/list', async (req, res) => {
    ...
    const subcats = cats.slice(0).map(i => i._id)
    cats.unshift({
      name: '热门',
  ...
  router.get('/heroes/init', async (req, res) => {
    ...
    for (let i of rawData) {
      ...
      await Hero.insertMany(i.heroes)
    }
  ...
  router.get('/heroes/list', async (req, res) => {
    ...
    cats.unshift({
      name: '热门',
      heroList: await Hero.find().where({categories:{$in: subcats}}).populate('categories').limit(10).lean()
    })
    cats.map(i => {
      i.heroList.map(hero => {
        hero.categoryName = (i.name === '热门') ? hero.categories[0].name : i.name
      })
    })
    res.send(cats)
  })
